...
export function getPrismaClient(config: GetPrismaClientOptions): any {
  class NewPrismaClient {
     ...
     */
    private async $executeRawInternal(string, ...values: sqlTemplateTag.RawValue[]) {
      let query = ''
      ...
      if (Array.isArray(string)) {
        ...
        const queryInstance = sqlTemplateTag.sqltag(string, ...values,)
        ...
        query = string[sqlOutput]
        parameters = {
          values: serializeRawParameters(string.values),
          __prismaRawParamaters__: true,
        }
      }
     ...
     * Executes a raw query. Always returns a number
     */
    private async $queryRawInternal(strings, ...values: sqlTemplateTag.RawValue[]) {
      let query = ''
      let parameters: any = undefined
